...
mod tests {
    ...
    #[actix_web::test]
    async fn multiple_memberships_per_item_with_different_parents() {
        let pool = test_pool().await;
        let owner = seed_owner(&pool).await;
        let item = seed_item(&pool, owner, "multi-parent").await;
        let parent1_id = item.id; // FK must reference existing item
        let session_id = Uuid::new_v4();

        // Seed one membership with parent_id.
        sqlx::query(
            "INSERT INTO memberships (id, item_id, parent_id, position, visible, created_at, updated_at, deleted_at)
             VALUES ($1, $2, $3, 0.0, true, now(), now(), NULL)"
        )
        ...
        // Sync and verify membership is returned.
        let req = aw_test::TestRequest::post()
            .uri("/api/sync")
            .cookie(actix_web::cookie::Cookie::build(
                "lister_session",
        ...
        // Count memberships for this item.
        let count = body.memberships.iter()
    ...
    #[test]
    fn membership_serde_large_position_serializes_correctly() {
        let m = Membership::new(Uuid::new_v4(), None, 1e10);
        let json = serde_json::to_string(&m).unwrap();
        assert!(json.contains("\"position\":10000000000.0") || json.contains("\"position\":1e10"),
            "position=1e10 should serialize correctly in JSON, got: {}", json);

        let deserialized: Membership = serde_json::from_str(&json).unwrap();
        assert!((deserialized.position - 1e10_f64).abs() < f64::EPSILON,
    ...
    #[test]
    fn membership_serde_position_half_serializes_correctly() {
        let m = Membership::new(Uuid::new_v4(), None, 0.5);
        let json = serde_json::to_string(&m).unwrap();
        assert!(json.contains("\"position\":0.5") || json.contains("\"position\":1e-1"),
            "position=0.5 should serialize correctly in JSON, got: {}", json);

        let deserialized: Membership = serde_json::from_str(&json).unwrap();
        assert!((deserialized.position - 0.5_f64).abs() < f64::EPSILON,
            "position=0.5 should round-trip correctly, got {}", deserialized.position);
    }

    #[test]
    fn membership_serde_position_quarter_serializes_correctly() {
        let m = Membership::new(Uuid::new_v4(), None, 0.25);
        let json = serde_json::to_string(&m).unwrap();
        assert!(json.contains("\"position\":0.25") || json.contains("\"position\":1e-1"),
            "position=0.25 should serialize correctly in JSON, got: {}", json);

        let deserialized: Membership = serde_json::from_str(&json).unwrap();
        assert!((deserialized.position - 0.25_f64).abs() < f64::EPSILON,
    ...
    #[test]
    fn membership_serde_position_two_serializes_correctly() {
        let m = Membership::new(Uuid::new_v4(), None, 2.0);
        let json = serde_json::to_string(&m).unwrap();
        assert!(json.contains("\"position\":2.0") || json.contains("\"position\":2"),
            "position=2.0 should serialize correctly in JSON, got: {}", json);

        let deserialized: Membership = serde_json::from_str(&json).unwrap();
        assert!((deserialized.position - 2.0_f64).abs() < f64::EPSILON,
    ...
    #[test]
    fn membership_serde_position_extremely_small_scientific_serializes_correctly() {
        let m = Membership::new(Uuid::new_v4(), None, 1e-9);
        let json = serde_json::to_string(&m).unwrap();
        // Serde may serialize as 0.000000001 or 1e-9
        assert!(json.contains("\"position\":0.000000001") || json.contains("\"position\":1e-9"),
            "position=1e-9 should serialize correctly in JSON, got: {}", json);

        let deserialized: Membership = serde_json::from_str(&json).unwrap();
        assert!((deserialized.position - 0.000000001_f64).abs() < f64::EPSILON,
            "position=1e-9 should round-trip correctly, got {}", deserialized.position);
    }

    #[test]
    fn membership_serde_position_ultra_small_scientific_serializes_correctly() {
        let m = Membership::new(Uuid::new_v4(), None, 1e-12);
        let json = serde_json::to_string(&m).unwrap();
        // Serde may serialize as 0.000000000001 or 1e-12
        assert!(json.contains("\"position\":0.000000000001") || json.contains("\"position\":1e-12"),
            "position=1e-12 should serialize correctly in JSON, got: {}", json);

        let deserialized: Membership = serde_json::from_str(&json).unwrap();
        assert!((deserialized.position - 0.000000000001_f64).abs() < f64::EPSILON,
